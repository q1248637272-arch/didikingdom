...
const webPort = 8806;
const cdpPort = 9256;
const url = `http://127.0.0.1:${webPort}/?v56-showtime=1`;
const profile = join(root, "tmp", `edge-v56-showtime-${Date.now()}`);
const desktopShot = join(root, "verification-v56-showtime-local.png");
const mobileShot = join(root, "verification-v56-showtime-mobile-local.png");
...
try {
  ...
  const seeded = await evaluate(client, `(() => {
    ...
    return {
      ...
      floor: {
        ...
        heat: Math.round(floor.showtime?.heat || 0),
        beat: floor.showtime?.beat || "",
        reactions: floor.showtime?.reactions || 0,
        earned: floor.showtime?.earned || 0,
        globalReactions: state.stats.showtimeReactionsDone || 0,
  ...
  const desktop = await evaluate(client, `(async () => {
    ...
    const cssText = await fetch('overrides.css?v=56', { cache: 'no-store' }).then((response) => response.text());
    const appText = await fetch('app.js?v=56', { cache: 'no-store' }).then((response) => response.text());
    ...
    return {
      appScript: Boolean(document.querySelector('script[src="app.js?v=56"]')),
      css: Boolean(document.querySelector('link[href="overrides.css?v=56"]')),
      swv56: swText.includes('little-depths-v56') && swText.includes('app.js?v=56'),
      cssv56: cssText.includes('showtime-meter') && cssText.includes('data-showtime-heat'),
      appv56: appText.includes('SHOWTIME_BEATS') && appText.includes('pulseShowtimeReaction') && appText.includes('showtimeReactionsDone'),
      ...
      heatPanels: document.querySelectorAll('.showtime-panel[data-heat]').length,
      meters: document.querySelectorAll('.showtime-meter').length,
      beatFloors: document.querySelectorAll('.floor.showtime-active[data-showtime-beat]').length,
      heatFloors: document.querySelectorAll('.floor.showtime-active[data-showtime-heat]').length,
  ...
  const assertions = {
    loadedv56: desktop.appScript && desktop.css && desktop.swv56 && desktop.cssv56 && desktop.appv56 && mobile.appScript && mobile.css,
    saveVersion: seeded.version === 11,
    showStarted: seeded.after >= seeded.before + 1 && seeded.floor.active && seeded.floor.applause > 0,
    showtimeHeat: seeded.floor.heat > 0,
    showtimeBeat: ["opening", "twist", "finale"].includes(seeded.floor.beat),
    showtimeReactions: seeded.floor.reactions >= 1 && seeded.floor.globalReactions >= 1,
    showtimeEarned: seeded.floor.earned >= seeded.floor.applause,
    ...
    visibleShowtimeUi: desktop.activeFloors >= 1 && desktop.activePanels >= 1 && desktop.heatPanels >= 1 && desktop.meters >= 1 && desktop.beatFloors >= 1 && desktop.heatFloors >= 1 && desktop.showtimeTags >= 1,
    ...
    mobileShowtimeVisible: mobile.activeFloors >= 1 && mobile.activePanels >= 1 && mobile.heatPanels >= 1 && mobile.meters >= 1 && mobile.beatFloors >= 1 && mobile.heatFloors >= 1 && mobile.showtimeTags >= 1 && mobile.kingdomWidth > 0 && mobile.bodyWidth > 0,
  ...
  if (failed.length) {
    throw new Error(`v56 verification failed: ${failed.map(([key]) => key).join(", ")}`);
  }
} catch (error) {
  edge.kill();
  await new Promise((resolveClose) => server.close(resolveClose));
