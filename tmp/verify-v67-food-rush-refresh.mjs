...
const webPort = 8817;
const cdpPort = 9267;
const url = `http://127.0.0.1:${webPort}/?v67-food-rush-refresh=1`;
const profile = join(root, "tmp", `edge-v67-food-rush-refresh-${Date.now()}`);
const desktopShot = join(root, "verification-v67-food-rush-refresh-local.png");
const mobileShot = join(root, "verification-v67-food-rush-refresh-mobile-local.png");
...
try {
  ...
  const seeded = await evaluate(client, `(() => {
    ...
    const servingQuest = state.quests.find((entry) => entry.id === "serving_line");
    return {
      ...
      beforeCourses,
      afterCourses: state.stats.foodRushCoursesDone || 0,
      coinsBeforeQuestReady,
      coinsAfterQuestReady: state.coins,
      gemsBeforeQuestReady,
      gemsAfterQuestReady: state.gems,
      servingQuestReady: Boolean(servingQuest?.ready),
      servingQuestClaimed: Boolean(servingQuest?.claimed),
      servingQuestButton: Boolean(document.querySelector('[data-action="claim-quest"][data-quest-id="serving_line"]')),
      ...
      floor: {
        ...
        courses: floor.foodRush?.courses || 0,
        course: floor.foodRush?.course || "",
        servicePulse: Number(floor.foodRush?.servicePulse || 0),
        tableFocus: Number(floor.foodRush?.tableFocus || 0),
  ...
  const desktop = await evaluate(client, `(async () => {
    ...
    const cssText = await fetch('overrides.css?v=67', { cache: 'no-store' }).then((response) => response.text());
    const appText = await fetch('app.js?v=67', { cache: 'no-store' }).then((response) => response.text());
    const artResponse = await fetch('assets/art/room-food-v3.webp', { cache: 'no-store' });
    const floor = state.floors.find((entry) => entry.type === 'food');
    const room = document.querySelector('.floor[data-type="food"] .room');
    return {
      appScript: Boolean(document.querySelector('script[src="app.js?v=67"]')),
      css: Boolean(document.querySelector('link[href="overrides.css?v=67"]')),
      swv67: swText.includes('little-depths-v67') && swText.includes('app.js?v=67') && swText.includes('room-food-v3.webp'),
      cssv67: cssText.includes('food-rush-panel') && cssText.includes('food-rush-service-layer') && cssText.includes('room-food-v3.webp'),
      appv67: appText.includes('startFoodRush') && appText.includes('foodRushCoursesDone') && appText.includes('renderFoodRushServiceLayer') && appText.includes('serving_line'),
      foodArt: getComputedStyle(room).getPropertyValue('--room-art').includes('room-food-v3.webp'),
      foodArtServed: artResponse.ok && (artResponse.headers.get('content-type') || '').includes('image'),
      ...
      readouts: document.querySelectorAll('.food-rush-readout b').length,
      courseRows: document.querySelectorAll('.food-rush-course-row i').length,
      serviceLayers: document.querySelectorAll('.food-rush-service-layer').length,
      serviceRails: document.querySelectorAll('.food-rush-service-rail').length,
      courseDots: document.querySelectorAll('.food-rush-course-stack i.lit').length,
      tableMarkers: document.querySelectorAll('.food-rush-table-lane b.active').length,
      ...
      serviceText: document.querySelector('.food-rush-panel')?.textContent || "",
  ...
  const afterClaim = await evaluate(client, `(() => {
    const button = document.querySelector('[data-action="claim-quest"][data-quest-id="serving_line"]');
    const beforeCoins = state.coins;
    const beforeGems = state.gems;
    button?.click();
    const quest = state.quests.find((entry) => entry.id === "serving_line");
    return {
      ...
      buttonGone: !document.querySelector('[data-action="claim-quest"][data-quest-id="serving_line"]'),
  ...
  const mobile = await evaluate(client, `(() => {
    ...
    return {
      ...
      panelNoOverflow: document.querySelector('.food-rush-panel')?.scrollWidth <= document.querySelector('.food-rush-panel')?.clientWidth + 1,
    };
  })()`);
  ...
  const assertions = {
    loadedv67: desktop.appScript && desktop.css && desktop.swv67 && desktop.cssv67 && desktop.appv67 && mobile.appScript && mobile.css,
    saveVersion: seeded.version === 22,
    ...
    rushCourses: seeded.afterCourses >= seeded.beforeCourses + 1 && seeded.floor.courses >= 1 && ["starter", "pot", "share", "finale"].includes(seeded.floor.course),
    manualQuestReady: seeded.servingQuestReady && !seeded.servingQuestClaimed && seeded.servingQuestButton && seeded.gemsAfterQuestReady === seeded.gemsBeforeQuestReady,
    manualQuestClaim: afterClaim.hadButton && afterClaim.afterCoins === afterClaim.beforeCoins + 460 && afterClaim.afterGems === afterClaim.beforeGems + 2 && afterClaim.claimed && !afterClaim.ready && afterClaim.buttonGone && afterClaim.log.includes("领取任务"),
    rushStates: ["seating", "serving", "sharing", "lastcall"].includes(seeded.floor.pace) && seeded.floor.heat > 0 && seeded.floor.earned > 0 && seeded.floor.servicePulse >= 0,
    ...
    visibleRushUi: desktop.activeFloors >= 1 && desktop.activePanels >= 1 && desktop.pacePanels >= 1 && desktop.meters >= 1 && desktop.readouts >= 3 && desktop.courseRows >= 4 && desktop.paceFloors >= 1 && desktop.heatFloors >= 1 && desktop.rushTags >= 1,
    visibleServiceLayer: desktop.serviceLayers >= 1 && desktop.serviceRails >= 1 && desktop.courseDots >= 1 && desktop.tableMarkers >= 2 && desktop.serviceText.includes("桌次"),
    visibleLifeMotion: desktop.lifeVisitors >= 2 && desktop.needBadges >= 4 && desktop.socialPairs >= 1,
    roomArt: desktop.foodArt && desktop.foodArtServed,
    ...
    mobileRushVisible: mobile.activeFloors >= 1 && mobile.activePanels >= 1 && mobile.pacePanels >= 1 && mobile.meters >= 1 && mobile.readouts >= 3 && mobile.courseRows >= 4 && mobile.serviceLayers >= 1 && mobile.serviceRails >= 1 && mobile.tableMarkers >= 2 && mobile.paceFloors >= 1 && mobile.heatFloors >= 1 && mobile.rushTags >= 1 && mobile.kingdomWidth > 0 && mobile.bodyWidth > 0 && mobile.noDocumentOverflow && mobile.panelNoOverflow,
  ...
  const summary = { url, desktopShot, mobileShot, seeded, afterClaim, desktop, mobile, assertions, errors };
  console.log(JSON.stringify(summary, null, 2));
  if (failed.length) {
    throw new Error(`v67 verification failed: ${failed.map(([key]) => key).join(", ")}`);
  }
} catch (error) {
  edge.kill();
  await new Promise((resolveClose) => server.close(resolveClose));
